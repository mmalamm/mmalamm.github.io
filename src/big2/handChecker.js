    ...
    case 5:
      const comboChecker = (() => {
        const isStraight = (input => {
            ...
            const ords = input.sort((a, b) => a.ord - b.ord).map(c => c.value);
            const i = checker.indexOf(uniqs[0]);
            const j = checker.indexOf(ords[0]);
            return (
        ...
        if (isStraight) {
          const mainCard = arr.filter(c => c._rank === maxRank(arr))[0];
          const str = mainCard._rank;
          return {
            strength: str + 400,
        ...
        if (isHouse) {
          const trip = countChecker(uniqs[0], 3) ? uniqs[0] : uniqs[1];
          const str = maxRank(arr.filter(c => c.value === trip));
          return {
            strength: str + 600,
            name: `House of ${trip}`
          };
        ...
        if (isBomb) {
          const quad = countChecker(uniqs[0], 4) ? uniqs[0] : uniqs[1];
